...
if [ $stage -le 1 ]; then
  ...
  # nets.
  # pk2743: We only need the test clean and test other datasets
...
if [ $stage -le 2 ]; then
  ...
  # pk2743: again, we only need the test clean and test other datasets
  for part in test-clean test-other; do
...
if [ $stage -le 18 ]; then
  ...
  for test in test_clean test_other; do
      steps/decode_fmllr.sh --nj 20 --cmd "$decode_cmd" \
                            exp/tri3/graph data/$test exp/tri3/decode_tgsmall_$test
